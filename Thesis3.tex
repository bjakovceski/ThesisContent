...
For instance types file we modified the algorithm for creating a data tree. Here because of lower range data we have created only files from "a-z", of coarse special characters files and other file.
...
This is our main experiment where other experiments will be compared with this one. This model is trained with top 300 Wikipedia abstracts, for every domain, based on PageRank\cite{wiki:PR}. The text is annotated in coarse grained and has around 2300 annotated words in total.
	First experiment that we do with this model is that we run it with the same text that model is created in coarse grained. Results are not bad at all, we are above 95\% (see Table~\ref{table:All3domainsWithAll3DomiansTop300Coarse}), which is great number for such middle weight model. But let see how model will behaves when we tested with abstracts for every domain.
	
	Table~\ref{table:All3domainsWithPoliticsTop300Coarse} shows the output of model when is tested with abstracts from a "POLITICS" domain. As we said in Section~\ref{StanfordNERoutput} this type of abstract has the biggest word annotation. In this experiment, trained model annotated words with a "TRANSPORTATION" domain. 
	
	Table~\ref{table:All3domainsWithSportTop300Coarse} gives us results from abstracts from "SPORT" domain. From the table we can see that our trained model annotated some words with a "POLITICS" or "TRANSPORTATION", even those that our test file contains only abstracts from "SPORT" domains and words has only "SPORT" type.
	
	Table~\ref{table:All3domainsWithTransportationTop300Coarse} provide outcome with testing with abstracts only from "TRANSPORTATION" domain. Also here the trained model annotated words with a "SPORT" type. 
	...
	\begin{table}[H]\centering
		\label{table:All3domainsWithAll3DomiansTop300Coarse}
	...
	\begin{table}[H]\centering
		\label{table:All3domainsWithPoliticsTop300Coarse}
		\begin{tabular}{|l|l|l|l|}
			\hline {\textbf{Entity}} & {\textbf{P}} & {\textbf{R}} & {\textbf{F1}}\\\hline
	...
	\begin{table}[H]\centering
		\label{table:All3domainsWithSportTop300Coarse}
		\begin{tabular}{|l|l|l|l|}
			\hline {\textbf{Entity}} & {\textbf{P}} & {\textbf{R}} & {\textbf{F1}}\\\hline
	...
	\begin{table}[H]\centering
		\label{table:All3domainsWithTransportationTop300Coarse}
		\begin{tabular}{|l|l|l|l|}
			\hline {\textbf{Entity}} & {\textbf{P}} & {\textbf{R}} & {\textbf{F1}}\\\hline
